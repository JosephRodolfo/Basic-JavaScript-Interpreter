...
impl FunctionDeclaration {
    pub fn create_function_declaration(program: String) -> FunctionDeclaration {
        ...
        let params_arr =
            FunctionDeclaration::create_comma_seperated_array(params_string_arr).unwrap();
    ...
    pub fn create_interpreter_var(value: &FunctionDeclaration)-> Result<(String, Vars), String> {

        //function declarations cannot be reassigned; right now that's achieved by saving it with "const" but the error message will be wrong; need to solve
        let result_tuple = (value.identifier.evaluate().to_string(), Vars{kind:"const".to_string(), value: VarsEnum::Obj(VariableInitTypes::FunctionExpression(value.clone()))});
